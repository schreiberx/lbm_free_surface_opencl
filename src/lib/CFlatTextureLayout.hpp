...
public:
	...
	CVector<3,int> domain_cells;	///< domain cells for which the flat texture layout was created

	/**
	 * initialize flat textures
	 *
	 * \param p_domain_cells	domain cells of volume texture
	 */
	inline void init(CVector<3,int> &p_domain_cells)
	{
		init(p_domain_cells[0], p_domain_cells[1], p_domain_cells[2]);
	}
	...
	{
		domain_cells[0] = res_x;
		domain_cells[1] = res_y;
		domain_cells[2] = res_z;
		/*
		...
		tmp = ft_z_mod = CMath::sqrt<int>(res_z);
		...
		tmp = ft_y_mod = CMath::sqrt<int>(res_y);
		...
		tmp = ft_x_mod = CMath::sqrt<int>(res_x);

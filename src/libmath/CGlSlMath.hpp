...
namespace GLSL
{
					...
					)
	{
		...
		float div = 1.0/CMath::sqrt(x*x+y*y+z*z);

		x *= div;
		y *= div;
		z *= div;

		float mul = CMath::PI<float>()/180.0f;
		float c = CMath::cos(angle*mul);
		float s = CMath::sin(angle*mul);
	...
	{
		return GLSL::vec3(CMath::abs(v.data[0]), CMath::abs(v.data[1]), CMath::abs(v.data[2]));
	}

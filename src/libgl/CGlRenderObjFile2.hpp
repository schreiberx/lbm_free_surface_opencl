...
public:
	...
	)
	{
		...
		for(std::list<CObjFile::CGroup>::iterator i = cObjFile.groups.begin(); i != cObjFile.groups.end(); i++)
		{
			...
			// TODO: don't allocate texture buffer space if no texture coordinates are used in the whole .obj file
		...
		vao.bind();
			...
			buffer.bind();
			buffer.data(buffer_size, NULL);

#if LBM_OPENCL_FS_DEBUG
			GLfloat *d = new GLfloat[buffer_size/sizeof(GLfloat)];
			for (int i = 0; i < (int)(buffer_size/sizeof(GLfloat)); i++) d[i] = -1;
			buffer.subData(0, buffer_size, d);
			delete [] d;
#endif
			...
			for(std::list<CObjFile::CGroup>::iterator gi = cObjFile.groups.begin(); gi != cObjFile.groups.end(); gi++)
			{
				...
				if (obj_group.lin_texture_coords != NULL)
				...
				for(std::list<CObjFile::CGroupMaterialMesh>::iterator mi = obj_group.meshes.begin(); mi != obj_group.meshes.end(); mi++)
				{
					...
					{
						error << CError_Location << "material not found: " << obj_mesh.material->name << CError::endl;
						return;
					}
...
/**
 * BUG IN AMD DRIVER: exchange vertices and normals!
 */
			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, buf_vertices_start_ptr);
//			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, buf_normals_start_ptr);
			glEnableVertexAttribArray(0);


//			glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, buf_vertices_start_ptr);
	...
	void cleanup()
	{
		...
		vao.bind();
			buffer.bind();
			buffer.data(1, NULL);	// initialize with 1 - 0 doesn't work
			buffer.unbind();
		vao.bind();

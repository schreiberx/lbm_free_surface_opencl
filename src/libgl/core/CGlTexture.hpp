...
// proxy textures dont seem to work with current nvidia drivers
		...
		{
			error << "ERROR: unsupported target texture" << std::endl;
			return false;
		}

		if (x == 0 && y == 0)
		{
			error << "ERROR: failed to create texture (maybe not enough memory)" << std::endl;
			return false;
		}
#endif
...
// proxy textures dont seem to work with current nvidia drivers
	 ...
	 */
	inline void loadFromFile(const std::string &filename, GLuint target)
	{
		...
		if (!surface)
		{
			...
			std::cout << "SDL IMG LOAD ERROR: " << filename << std::endl;
			error << "Cannot load image \"" << filename << "\""<< SDL_GetError() << std::endl;
			return;
		}
		...
		else
		{
			...
			error << "Image has to be 3 or 4 bytes per pixel \"" << filename << "\""<< std::endl;
			return;
		}
	...
	{
		loadFromFile(filename, GL_TEXTURE_2D);
	}

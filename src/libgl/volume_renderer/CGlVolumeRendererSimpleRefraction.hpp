...
#ifndef CGL_VOLUME_RENDERER_REFRACTION_HPP
#define CGL_VOLUME_RENDERER_REFRACTION_HPP
...
/**
 * \brief OpenGL volume renderer using the equidistant step lengths and refractions
 */
class CGlVolumeRendererSimpleRefraction	:
...
public:
	 ...
	 */
	CGlVolumeRendererSimpleRefraction()
	{
		cGlProgram.initVertFragShadersFromDirectory("volume_renderer/simple_refraction");
...
//		light0_view_pos3_uniform.init(cGlProgram, "light0_view_pos3");
		...
		)
	{
		...
		texture_size_add_one[0] = (float)volume_texture.width+1.0f;
		texture_size_add_one[1] = (float)volume_texture.height+1.0f;
		texture_size_add_one[2] = (float)volume_texture.depth+1.0f;

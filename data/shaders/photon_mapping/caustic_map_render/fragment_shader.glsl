...
uniform vec3 photon_color;

//in vec3 asdf;

void main(void)
{
	frag_data = vec4(photon_color,point_alpha);
	return;
	...
	a.x = a.x*a.x+a.y*a.y;
	if (a.x > 1.0)	discard;
	frag_data = vec4(1.0, 1.0, 1.0, (1.0-sqrt(a.x))*alpha_attenuation);

#else
	// use max(alpha, 0) - otherwise we'll produce negative alpha values
	frag_data = vec4(1.0, 1.0, 1.0, max((1.0-length(gl_PointCoord*2.0-1.0))*alpha_attenuation, 0));
#endif
}

						...
						)
{
	...
	// load refracted/reflected pixels from cube map

	frag_data = blinnShadingVolume(
								texture(cube_map, model_normal_matrix3*exit_dir3),
	...
	{
		// compute FRESNEL FACTOR for reflected ray using SCHLICKs approximation.
		// the last term is computed between the angle of the view vector and the halfway vector.
		// for total reflections, the halfway vector coincides with the normal!!!
		float kasten_bier = 1.0f - max(dot(-enter_dir, iso_normal3), 0);
		float zwei_kaesten_bier = kasten_bier*kasten_bier;
		kasten_bier = zwei_kaesten_bier*zwei_kaesten_bier*kasten_bier;

		frag_data = mix(	frag_data,	// refracted data
							texture(cube_map, model_normal_matrix3*first_reflect_dir),	// reflected environment
...
void rayCast()
{
	...
	for (float step_fac = step_length*2.0; step_fac < max_dist; step_fac += step_length)
	{
...
#define prev_pos	_normal
			prev_pos = _pos + _dir*(step_fac-step_length);
			// abuse step_fac for prev_value
#define prev_value	step_fac
			prev_value = texture(volume_texture, prev_pos).r;

...
void main(void)
{
	...
	{
		vertex_pos = vec4(0,0,0,0);
		vertex_normal = vec4(0,0,0,0);
		return;
	}
#endif
	ivec3 pos;
...
#if 1
	if (axis == 0)		fpos.x += interp(ff000, getFluidFraction(pos.x+1, pos.y, pos.z));
	else if (axis == 1)	fpos.y += interp(ff000, getFluidFraction(pos.x, pos.y+1, pos.z));
	else				fpos.z += interp(ff000, getFluidFraction(pos.x, pos.y, pos.z+1));
#else
	switch(axis)
	{
		case 0:
...
#if 1
	/**
	 * use GLSL interpolated values which seem to be not accurate enough for driver version 195.30
	 */

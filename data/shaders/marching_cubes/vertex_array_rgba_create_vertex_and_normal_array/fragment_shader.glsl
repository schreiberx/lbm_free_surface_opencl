...
void main(void)
{
	...
	// TODO: be more cache efficient when creating the id
	// simple linear id creation
	uint linear_id = uint(flat_pos.y * VERTEX_ARRAY_TEXTURE_WIDTH + flat_pos.x);
	...
	vertex_pos.x = float(pos.x) + displacements_x[vertex_id];
	vertex_pos.y = float(pos.y) + displacements_y[vertex_id];
	vertex_pos.z = float(pos.z) + displacements_z[vertex_id];
	vertex_pos.w = 1;
...
//	int dz = displ_interpol_z[interpolation_axis];
	...
	vertex_pos.x += float(displ.x)*displacement;
	vertex_pos.y += float(displ.y)*displacement;
	vertex_pos.z += float(displ.z)*displacement;

	vertex_pos = vertex_fix_matrix4*vertex_pos;
	...
	vertex_normal = vec4(normalize(normal_fix_matrix3*normal), 0);
#endif
}
